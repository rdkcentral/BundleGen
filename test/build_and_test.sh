...
if [[ $2 == docker://** ]]; then
  ...
  bundlegen -vvv generate --appid ${TEMPLATE}-${APP_NAME} --searchpath templates --platform ${TEMPLATE} $2 ${TEMPLATE}-${APP_NAME} ${APPMETADATA} ${EXTRA_OPTIONS}
else
  ...
  echo "--> Generating runtime bundle..."
  rm -rf ./${TEMPLATE}-${APP_NAME}
  bundlegen -vvv generate --appid ${TEMPLATE}-${APP_NAME} --searchpath templates --platform ${TEMPLATE} oci:./oci-${APP_NAME}:latest ${TEMPLATE}-${APP_NAME} ${APPMETADATA} ${EXTRA_OPTIONS}
fi
