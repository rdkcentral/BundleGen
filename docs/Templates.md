...
* `gpu`
  ...
  * `extraMounts` (array, OPTIONAL) Array of *mount* objects that will be added to the config to allow mounting specific graphics sockets or other files into a container. Each mount object should be an OCI `Mount` object as defined [here](https://github.com/opencontainers/runtime-spec/blob/master/config.md#mounts). Use `X-dobby.optional` to make the mount optional. Use `X-mount.mkdir` in combination with cmdline parameter `--createmountpoints` to get this mount entry created as directory inside the generated rootfs. Not setting `X-mount.mkdir` in combination with `--createmountpoints` will create an empty file in the rootfs for the mount entry. Setting option `X-mount.no` will only create the empty file or directory inside the rootfs and will not add a mount entry to the configuration. This is useful for read-only rootfs and when dynamically added mount entries are expected (like the westeros socket).
...
* `journaldOptions` (object, OPTIONAL). Journald logging settings. The object should match the
  `journaldOptions` section of the Logging plugin as defined
  [here](https://github.com/rdkcentral/Dobby/tree/master/rdkPlugins/Logging/README.md)
* `ipc` (object, OPTIONAL). IPC plugin settings. The object should match the `data` section of the IPC
  plugin as defined [here](https://github.com/rdkcentral/Dobby/tree/master/rdkPlugins/IPC/README.md)
* `minidump` (object, OPTIONAL). Minidump settings. The object should match the `data` section of the
  Minidump plugin as defined
  [here](https://github.com/rdkcentral/Dobby/blob/master/rdkPlugins/Minidump/source/README.md)
* `oomcrash` (object, OPTIONAL). OOM Crash settings. The object should match the `data` section of the
  OOMCrash plugin as defined
  [here](https://github.com/rdkcentral/Dobby/blob/master/rdkPlugins/OOMCrash/README.md)
